...
static NSString * const GETAPPiWineAPIEngineURL = @"";

@implementation GETAPIEngine

+ (instancetype)sharedInstance {
    static GETiWineAPIEngine *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // Initialize the session
        _sharedInstance = [[GETiWineAPIEngine alloc] initWithBaseURL:[NSURL URLWithString:GETAPPiWineAPIEngineURL]];
    });
...
- (instancetype)initWithBaseURL:(NSURL *)url sessionConfiguration:(NSURLSessionConfiguration *)configuration
{
    ...
    [self.requestSerializer setTimeoutInterval:250];
...
- (void)fetchObjectsForClass:(NSString *)className withParameters:(NSDictionary *)parameters onCompletion:(FetchObjectsCompletionBlock)completionBlock
{
    ...
    NSString *agent_id = [[GETiWineModel sharedInstance] agentIdInSession] ?: @"";
    NSString *uuid = [[NSUserDefaults standardUserDefaults] objectForKey:@"uuid"];
    NSString *hash = [self generateHashWithParameters:@[name, uuid, version]];
    NSString *ios = [[UIDevice currentDevice] systemVersion];
    NSMutableDictionary *basicParameters = [NSMutableDictionary dictionaryWithObjectsAndKeys:version, @"version", agent_id, @"agent_id", uuid, @"uuid", hash, @"hash", ios, @"ios", nil];
...
{
    if([className isEqualToString:@"Multimedia"]){
        return @"multimedia";
    }
    else if([className isEqualToString:@"ProductMultimedia"]){
        return @"product_multimedia";
    }
    else if([className isEqualToString:@"WineryMultimedia"]){
        return @"winery_multimedia";
    }
    else if([className isEqualToString:@"ClientMultimedia"]){
        return @"client_multimedia";
    }
    else if([className isEqualToString:@"VisitMultimedia"]){
        return @"visit_multimedia";
    }
    else if([className isEqualToString:@"AgreementMultimedia"]){
        return @"agreement_multimedia";
    }
    else{
