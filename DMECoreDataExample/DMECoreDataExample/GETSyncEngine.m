...
-(void)downloadSyncEntitiesForSync:(RecieveObjectsCompletionBlock)completionBlock {
    [self.context performBlock:^{
        ...
        [[GETiWineAPIEngine sharedInstance] fetchEntitiesForSync:^(NSArray *objects, NSError *error) {
...
- (void)downloadJSONForRegisteredObjects:(RecieveObjectsCompletionBlock)completionBlock {
    [self.context performBlock:^{
        ...
        for (NSString *className in self.classesToSync) {
            ...
            [[GETiWineAPIEngine sharedInstance] fetchObjectsForClass:className withParameters:nil onCompletion:^(NSArray *objects, NSError *error) {
        ...
        [self saveContext:^(BOOL result) {
            if(result){
                ...
                if(self.classesToSync.count > 0 && self.initialSyncComplete){
                    [[GETiWineAPIEngine sharedInstance] pushEntitiesSynchronized:self.classesToSync onCompletion:^(NSDictionary *object, NSError *error) {
                        [self.context performBlock:^{
                            if(!error){
            ...
            if(objectsToCreate.count > 0){
                ...
                for (NSManagedObject *objectToCreate in objectsToCreate) {
                    ...
                    if(jsonString && jsonString.count > 0){
                        ...
                        [[GETiWineAPIEngine sharedInstance] pushObjectForClass:className parameters:jsonString files:filesURL onCompletion:^(NSDictionary *object, NSError *error) {
                            [self.context performBlock:^{
                                if(!error){
                                    if(object.count > 0){
            ...
            if(objectsToModified.count > 0){
                ...
                for (NSManagedObject *objectToModified in objectsToModified) {
                    ...
                    if(jsonString && jsonString.count > 0){
                        ...
                        [[GETiWineAPIEngine sharedInstance] updateObjectForClass:className withId:[objectToModified valueForKey:@"id"] parameters:jsonString files:filesURL onCompletion:^(NSDictionary *object, NSError *error) {
                            [self.context performBlock:^{
                                if(!error){
                                    if(object.count > 0){
                                        for (NSString* key in object) {
            ...
            if(objectsToDelete.count > 0){
                ...
                for (NSManagedObject *objectToDelete in objectsToDelete) {
                    ...
                    if(objectId && ![objectId isEqualToString:@""]){
                        ...
                        [[GETiWineAPIEngine sharedInstance] deleteObjectForClass:className withId:[objectToDelete valueForKey:@"id"] onCompletion:^(NSDictionary *object, NSError *error) {
                            [self.context performBlock:^{
